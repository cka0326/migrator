...
- **JSON**: `Header` → Upload JSON → `uploadService.processLineageUpload` → `ingestion.ingestLineageJSON`. Validates with `LineageExtractSchema` (`schema_version` must be `"1.0"`). Scopes every `dataset_id` to the canvas (`scope()`), **upserts** datasets (merges columns, including columns referenced only by edges), creates **STUB** nodes for referenced-but-undefined datasets, and writes everything in a single Dexie transaction. **Import is always additive**: existing metadata/columns/edges are never overwritten or deleted. Edges are written with **content-derived ids** via `bulkPut` (de-duped within the batch) so re-imports are **idempotent** (no duplicate connections). `processLineageUpload` never supersedes a same-named prior upload; the only intentional removal path is **Upload History → Delete** (`Repository.deleteUpload`).
  - **AI extraction from SAS/SQL**: the documented workflow is an external agent that emits a v1.0 JSON extract which the user uploads via Upload JSON. The agent contract, prompt, identity rules (`dataset_id` = `<SYSTEM>:<NAMESPACE>.<NAME>` UPPERCASE so it merges with existing tables), and a worked example live in **`EXTRACTION.md`**.
- **Excel**: `Header` → Upload Excel → `excelService.processExcelUpload`. Parses the MASTER registry (`TABLE_1..TABLE_15`, only rows with a `table_name`), per-table sheets, and the table/column connection sections. The downloadable templates in `public/templates/` are generated by the Python scripts in `scripts/` (`build_template.py`, `build_sample.py`) using `openpyxl` (needed for native dropdowns the `xlsx` lib can't emit). **If you change the Excel structure, update both `excelService.ts` and the Python scripts.**
- **Full workspace** backup: `lib/workspaceUtils.ts` (`exportWorkspaceToFile` / `importWorkspaceFromFile`) ↔ `Repository.getWorkspaceExport` / `importWorkspace`.
