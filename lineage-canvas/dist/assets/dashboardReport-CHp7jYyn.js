import{a as e,i as t,n,o as r,r as i,s as a}from"./index-CHOab2JV.js";var o=e=>String(e??``).replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#39;`})[e]),s=e=>e?e.namespace?`${e.namespace}.${e.name}`:e.name:`∅ deleted`;function c(e,t,n){let r=2*Math.PI*42,i=r*(1-Math.max(0,Math.min(100,e))/100);return`<svg viewBox="0 0 110 110" width="120" height="120" role="img" aria-label="${o(t)} ${e}%">
  ...
  </svg>`}function l(n){let r=e.reduce((e,t)=>e+n[t],0);if(r===0)return`<div class="muted">No mappings yet.</div>`;let a=0;return`<svg viewBox="0 0 100 16" width="100%" height="16" preserveAspectRatio="none">${e.filter(e=>n[e]>0).map(e=>{let s=n[e]/r*100,c=`<rect x="${a}" y="0" width="${s}" height="16" fill="${i[e]}"><title>${o(t[e])}: ${n[e]}</title></rect>`;return a+=s,c}).join(``)}</svg><div class="legend">${e.map(e=>`<span class="lg"><i style="background:${i[e]}"></i>${o(t[e])} ${n[e]}</span>`).join(``)}</div>`}function u(e){let t=e.trend;if(t.length<2)return`<div class="muted">Need at least two canvases for a trend.</div>`;let n=e=>36+(t.length===1?592/2:e/(t.length-1)*592),r=e=>12+160*(1-e/100),i=(e,i)=>`<path d="${t.map((t,i)=>`${i?`L`:`M`}${n(i).toFixed(1)} ${r(t[e]).toFixed(1)}`).join(` `)}" fill="none" stroke="${i}" stroke-width="2.5"/>${t.map((t,a)=>`<circle cx="${n(a).toFixed(1)}" cy="${r(t[e]).toFixed(1)}" r="3" fill="${i}"/>`).join(``)}`,a=[0,25,50,75,100].map(e=>`<line x1="36" y1="${r(e)}" x2="628" y2="${r(e)}" stroke="#eef2f7"/>
  ...
  <span class="lg"><i style="background:#2563eb"></i>Column coverage</span></div>`}function d(e){let r=e.status,a=new Map(e.nodes.map(e=>[e.datasetId,e])),u=r.perMapping.map(e=>`
    <tr>
      ...
      <td><span class="chip" style="background:${i[e.validationState]}22;color:#334155">${o(t[e.validationState])}</span></td>
    </tr>`).join(``);return`
  <section class="card">
    <h2>${o(e.canvas.name)}</h2>
    <div class="kpis">
      ${c(r.tableCoveragePct,`Tables mapped`,`#14b8a6`)}
      ${c(r.columnCoveragePct,`Columns mapped`,`#2563eb`)}
      <div class="stat"><div class="big">${Math.max(r.mappedLegacyCount,r.mappedTargetCount)}/${Math.max(r.legacyTableCount,r.targetTableCount)}</div><div class="lbl">Tables mapped</div></div>
      <div class="stat"><div class="big" style="color:${r.mismatchCount?`#dc2626`:`#0f172a`}">${r.mismatchCount}</div><div class="lbl">Type mismatches</div></div>
    </div>
    <div class="bar">${l(r.validationHistogram)}</div>
    ${r.perMapping.length?`<table><thead><tr>
